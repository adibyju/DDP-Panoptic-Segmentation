...
class SwinSpatialEncoder(nn.Module):
    def __init__(self, img_size=(128, 128), in_chans=10):
        super().__init__()
        ...
        # safe default for stage channels
        self.stage_channels = [128, 256, 512, 1024]

    def forward(self, x):
        B, T, C, H, W = x.shape
        # forward one frame to get number of outputs
        with torch.no_grad():
            sample_out = self.swin(x[:,0])
            num_stages = len(sample_out)
        feature_maps = [[] for _ in range(num_stages)]

        for t in range(T):
            frame = x[:, t, :, :, :]
            features = self.swin(frame)
            for i in range(num_stages):
                feature_maps[i].append(features[i].unsqueeze(1))

        feature_maps = [torch.cat(fm_list, dim=1) for fm_list in feature_maps]
        return feature_maps
...
class UTAE(nn.Module):
    def __init__(
        ...
        self.spatial_encoder = SwinSpatialEncoder()
        self.encoder_widths = self.spatial_encoder.stage_channels
    ...
    def forward(self, input, batch_positions=None, return_att=False):
        ...
        feature_maps = self.spatial_encoder(input)
        out = feature_maps[-1]
